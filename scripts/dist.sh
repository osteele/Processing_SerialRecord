...
project_file_src=library.properties
zip_src=target/SerialRecord-${version}-processing-library.zip

project_file_key=${S3_DIR}/${name}.txt
zip_key=${S3_DIR}/${name}.zip

aws s3 cp "$project_file_src" s3://${S3_BUCKET}/${project_file_key}
aws s3 cp "$zip_src" s3://${S3_BUCKET}/${zip_key}
